...
%Title of paper
\title{The stopping power of hydrogen in hafnium and \\
the importance of relativistic $4f$ electrons}
...
the $4f$-shell and the screening between $4f$ and $5p$ electrons to be 
decisive around the stopping maximum. Present theoretical and 
experimental results are in excellent agreement in the energy region 
of the new measurements. However, our theoretical stopping 
...
energies. Our calculations suggest the stopping maximum to be higher and 
shifted to lower energies than these previous predictions. Future 
measurements below 100 keV would be necessary for a better understanding 
of the stopping power around the maximum and below.
\end{abstract}
...
loss process is essential not only to have a better comprehension of the 
physics behind the fundamental interactions but also because it plays 
a vital role in many applied fields such as materials science, 
...
above mentioned metal-oxide films depend on their thickness, which is 
often measured by using Rutherford Backscattering 
Spectrometry~\cite{Alfassi01,Tesmer01}. This method relies heavily on 
...
of the $4f$-shell to be decisive even at impact energies around the 
...
for the FEG; in low energy region, the screened potential with cusp 
condition model (SPCC)~\cite{mon17}, which is non-linear binary 
formalism, and the Mermin-Lindhard dielectric formalism 
(ML)~\cite{Mermin} for energies around the stopping maximum and above. 
Our model requires the relativistic wave functions and binding energies 
of Hf and considers four electrons per atom in the FEG~\cite{mendez2019}. 
Hafnium is particularly interesting since the filled $4f$-subshell (with 
14 electrons) is the main contributor below the FEG, causing the 
...
considered and found to play a significant role within the SLPA 
calculations.
...
only experimental values measured by Sirotinin {\it et al.}~\cite{Sirotinin}  
...
package~\cite{Ziegler01} and the ICRU-49 tabulation~\cite{ICRU49}. 
...
the hafnium sample, which is subsequently used to determine the energy 
loss in the foil.}
\label{F01}
\end{figure}
...
electrostatic lenses and collimators onto a thin Au/SiO$_2$ sample, 
which is used as a scattering center. This sample was placed in the 
center of an RBS/C scattering chamber, where a high vacuum (pressure of 
...
to account for the non-linear dependence on ion energy of stopping 
...
with open circles. (b) Corresponding relative errors with respect to 
experimental data.}
\label{Binding_E}
\end{figure*}
%------------------------------------------------------------------------
...
More detail of the theoretical binding energies is given in 
...
naturally within a many-electron model, such as the SLPA.

%=======================================================================
\section{Analysis of the results and discussion}
\label{discussion}

The present data are displayed in Table~\ref{table01}. As can be 
observed, an overall relative uncertainty 
...
stopping maximum at an impact energy of 115 keV. The stopping maximum is 
...
with the stopping maximum at the same impact energy but $4\%$ higher. 

Future experiments for impact energies below 100 keV would be essential 
to complete this study on the stopping of Hf for protons. In the energy 
region $80-500$ keV, only the measurements by Sirotinin's 
group~\cite{Sirotinin} from 1984 are available. SRIM-2013 includes these 
...
(i.e., $50-200$ keV), and at low impact energies. 
...
calculations of stopping in Hf to take into account relativistic effects 
in the atomic structure and screening among electrons in a consistent 
way, from very low to high impact energies. Future experiments for 
